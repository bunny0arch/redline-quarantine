...
export class Input {
 ...
 const joyEnd=e=>{if(this.joyId!==e.pointerId)return;this.joyId=null;this.x=0;this.y=0;knob.style.transform='';joy.classList.remove('pressed');};joy.addEventListener('pointerup',joyEnd);joy.addEventListener('pointercancel',joyEnd);joy.addEventListener('lostpointercapture',e=>joyEnd(e));
 ...
 const aimEnd=e=>{if(this.aimId!==e.pointerId)return;this.aimId=null;this.fire=false;fire.classList.remove('pressed');};fire.addEventListener('pointerup',aimEnd);fire.addEventListener('pointercancel',aimEnd);fire.addEventListener('lostpointercapture',e=>aimEnd(e));
 }
