...
export class Game {
 ...
 tick(dt,input={x:0,y:0,fire:false}){
  ...
  for(let i=0;i<s.companions.length;i++){const a=s.companions[i];if(a.hp<=0)continue;a.cd=Math.max(0,a.cd-dt);let target=live.filter(e=>dist(a,e)<180&&clearLine(w,a,e)).sort((x,y)=>dist(a,x)-dist(a,y))[0];if(!target&&w.boss.active&&!w.boss.dead&&dist(a,w.boss)<280)target=w.boss;
   if(target){const reach=target===w.boss?58:34;if(dist(a,target)>reach)steer(w,a,target,this.flow,165,dt);else if(a.cd<=0){const dmg=a.weapon?C.weaponDamage:C.companionDamage;if(target===w.boss)this.damageBoss(dmg);else this.hitEnemy(target,dmg);a.cd=.85;this.sound('melee');}}
  ...
  for(const b of this.bullets){b.life-=dt;const steps=Math.ceil(Math.hypot(b.vx,b.vy)*dt/7);for(let i=0;i<steps&&b.life>0;i++){b.x+=b.vx*dt/steps;b.y+=b.vy*dt/steps;if(solid(w,b.x,b.y)){b.life=0;this.effect(b.x,b.y,'#a4aca2',9);break;}
    if(b.hostile){if(dist(b,p)<17){this.damage(p,24);b.life=0;}}else{const e=live.find(e=>e.alive&&dist(e,b)<18);if(e){this.hitEnemy(e,C.bulletDamage);b.life=0;}else if(w.boss.active&&!w.boss.dead&&dist(b,w.boss)<37){this.damageBoss(C.bulletDamage);b.life=0;this.effect(b.x,b.y);}}
 ...
 damageBoss(amount){const b=this.s.world.boss;if(b.dead)return;if(b.phase<4&&b.hp-amount<=C.bossHP*.2){this.bossFinale();return;}b.hp-=amount;if(b.hp<=0)this.win();}
 bossFinale(){const s=this.s,w=s.world,b=w.boss;if(b.dead||b.phase===4)return;b.phase=4;b.hp=Math.max(b.hp,C.bossHP*.55);b.finalTriggered=true;this.sound('boss-phase');this.effect(b.x,b.y,'#ffcf66',260);this.message('THE HEART REJECTS DEATH · FINAL CONTAINMENT BREACH');const first=w.enemies.reduce((n,e)=>Math.max(n,Number.isInteger(e.id)?e.id:n),-1)+1;const positions=[[90,16],[96,16],[102,16],[88,26],[104,26],[90,36],[96,36],[102,36]];positions.forEach(([x,y],i)=>w.enemies.push({id:first+i,x:x*48+24,y:y*48+24,hp:40,alive:true,room:13,cd:0,alert:6}));this.sound('boss-awaken');}
 ...
 if(b.phase<4&&b.hp<=C.bossHP*.2)this.bossFinale();const phase=b.phase===4?4:b.hp<C.bossHP*.25?3:b.hp<C.bossHP*.5?2:1;if(phase!==b.phase){b.phase=phase;this.sound('boss-phase');this.effect(b.x,b.y,phase===3?'#ff9f58':'#ff5b46',phase===3?210:170);this.message(phase===2?'THE HEART SHEDS ITS SKIN · Keep moving.':'THE HEART IS BREAKING · It calls the dead back.');if(phase===3&&!b.summoned){b.summoned=true;const first=w.enemies.reduce((n,e)=>Math.max(n,Number.isInteger(e.id)?e.id:n),-1)+1;for(let i=0;i<4;i++)w.enemies.push({id:first+i,x:(93+i*2)*48+24,y:(20+(i%2)*10)*48+24,hp:40,alive:true,room:13,cd:0,alert:6});this.message('REINFORCEMENTS · The containment walls are opening.');}}
 b.clock-=dt;const finale=b.phase===4,enraged=b.phase>=3,stalkSpeed=finale?132:enraged?108:b.phase===2?80:62,stalkDelay=finale?.75:enraged?1.1:b.phase===2?1.4:2,restDelay=finale?.65:enraged?.9:1.5;
 if(b.mode==='stalk'){steer(w,b,p,null,stalkSpeed,dt);if(b.clock<=0){b.attack=(b.attack+1)%(finale?5:enraged?4:3);b.tx=p.x;b.ty=p.y;b.mode='warn';b.clock=finale?.6:enraged?.75:1;this.sound('warning');}}
 else if(b.mode==='warn'&&b.clock<=0){if(b.attack===0){b.mode='charge';b.clock=finale?.42:enraged?.52:.65;const d=Math.hypot(b.tx-b.x,b.ty-b.y)||1;b.vx=(b.tx-b.x)/d*(finale?680:enraged?590:490);b.vy=(b.ty-b.y)/d*(finale?680:enraged?590:490);}
  else if(b.attack===1){this.effect(b.x,b.y,'#ff5b46',finale?220:enraged?190:170);for(const a of [p,...s.companions])if(a.hp>0&&dist(a,b)<(finale?195:enraged?175:155)&&clearLine(w,b,a))this.damage(a,a.armor?finale?35:enraged?30:25:finale?56:enraged?48:40);b.mode='rest';b.clock=restDelay;}
  else if(b.attack===2){const count=finale?20:enraged?16:10,speed=finale?270:enraged?230:180;for(let i=0;i<count;i++){const a=i*Math.PI*2/count;this.bullets.push({x:b.x,y:b.y,vx:Math.cos(a)*speed,vy:Math.sin(a)*speed,hostile:true,life:3});}if(enraged)this.sound('boss-burst');b.mode='rest';b.clock=restDelay;}
  else if(b.attack===3&&finale){const base=Math.atan2(p.y-b.y,p.x-b.x),count=8;for(let i=0;i<count;i++){const a=base+(i-(count-1)/2)*.16;this.bullets.push({x:b.x,y:b.y,vx:Math.cos(a)*320,vy:Math.sin(a)*320,hostile:true,life:3});}this.effect(b.x,b.y,'#ffb45e',150);this.sound('boss-burst');b.mode='rest';b.clock=restDelay;}
  else if(b.attack===3){const count=16,speed=230;for(let i=0;i<count;i++){const a=i*Math.PI*2/count;this.bullets.push({x:b.x,y:b.y,vx:Math.cos(a)*speed,vy:Math.sin(a)*speed,hostile:true,life:3});}this.sound('boss-burst');b.mode='rest';b.clock=restDelay;}
  else{for(let i=0;i<6;i++){const a=Math.atan2(p.y-b.y,p.x-b.x)+(i-2.5)*.22;this.bullets.push({x:b.x,y:b.y,vx:Math.cos(a)*390,vy:Math.sin(a)*390,hostile:true,life:3});}this.message('FINAL PATTERN · Break line of sight.');this.sound('boss-burst');b.mode='rest';b.clock=restDelay;}}
 else if(b.mode==='charge'){move(w,b,b.vx*dt,b.vy*dt,30);for(const a of [p,...s.companions])if(a.hp>0&&dist(a,b)<55&&(a===p||!b.struck?.includes(a.id))){this.damage(a,a.armor?finale?35:enraged?30:24:finale?50:enraged?44:35);if(a!==p)(b.struck??=[]).push(a.id);}if(b.clock<=0){b.mode='rest';b.clock=restDelay;b.struck=[];}}
